...
private:
    void imuHandler(const sensor_msgs::ImuConstPtr& imu_msg) {
        ...
        sensor_msgs::Imu thisImu = imuConverter(*imu_msg);      // THIS TRANSFORMS IMU DATA INTO LIDAR FRAME!!!!!!!!!!!!!!!!
    ...
    void blocMainProcess(const block_localization::cloud_infoConstPtr& cloudinfo_msg) {
        ...
        if(!systemInitialized) {
            ...
            // initial pose
            NODELET_INFO("Before modifying prevPose_: x=%f, y=%f, z=%f", prevPose_.x(), prevPose_.y(), prevPose_.z());
...
private:
    ...
    boost::shared_ptr<pcl::Filter<PointT>> downsample_filter;
